...
from juriscribe.stress_evidence import INSTANCE_CLAIM_SCOPE, build_mutation_coverage_evidence, build_saturation_coverage_evidence, validate_mutation_evidence, validate_saturation_evidence
...
class RuntimeSemanticsV12Tests(unittest.TestCase):
    def _semantic_fixture(self):
        ...
        relations = [{"id":f"R-{index}","source":canonical_unit["id"],"predicate":"CONDITIONS","target":unit["id"],"material":True} for index, unit in enumerate(candidate_units, 1)]
        state = SimpleNamespace(corpus=[{"source_id":"canon","role":"canonical_material","digest":text_digest(canonical_text)},{"source_id":"cand","role":"candidate_material","digest":text_digest(candidate_text)}],epistemic_units=units,relations=relations,reticulum={"status":"PASS","digest":"RET-CURRENT"},strategy={"consolidation":{"inventories":{"canon":canonical,"cand":candidate},"refactoring_contract":{"status":"READY","digest":"PLAN-CURRENT"}}})
        refined_inventory = build_lossless_inventory(refined_text, source_id="cand", role="candidate_material")
        projected_units = [{**unit,"object_id":obj["id"],"text":obj["text"]} for unit,obj in zip(candidate_units, refined_inventory["objects"])]
    ...
    def test_stress_evidence_scopes_volume_and_binds_class_coverage(self):
        mutation = {"schema":MUTATION_SCHEMA,"plan_digest":"PLAN","reticulum_digest":"RET","cases":10_000_000,"families":sorted(REQUIRED_MUTATION_FAMILIES),"failures":0}
        mutation["coverage_evidence"] = build_mutation_coverage_evidence(cases=10_000_000,class_counts={"valid":1_000_000,"adversarial":9_000_000})
        ...
        spoofed = copy.deepcopy(mutation); spoofed["coverage_evidence"]["class_counts"]["adversarial"] -= 1
        self.assertFalse(validate_mutation_evidence(spoofed, plan_digest="PLAN", reticulum_digest="RET")[0])
        saturation = {"schema":SATURATION_SCHEMA,"plan_digest":"PLAN","no_novelty_tail":1000,"no_better_compression_tail":1000,"canonical_unchanged":True}
        saturation["coverage_evidence"] = build_saturation_coverage_evidence(probes=2000,class_counts={"no_novelty":1000,"no_better_compression":1000})
        self.assertTrue(validate_saturation_evidence(saturation, plan_digest="PLAN")[0])
        asserted = copy.deepcopy(saturation); asserted["semantic_recall"] = 1.0; asserted["relation_recall"] = 1.0
    ...
    def test_malformed_stress_evidence_fails_closed(self):
        mutation = {"schema":MUTATION_SCHEMA,"plan_digest":"PLAN","reticulum_digest":"RET","cases":10_000_000,"families":sorted(REQUIRED_MUTATION_FAMILIES),"failures":0,"coverage_evidence":{"schema":"juriscribe-mutation-coverage-evidence/v1","instance_claim_scope":INSTANCE_CLAIM_SCOPE,"instances":10_000_000,"equivalence_classes":2,"class_counts":{"valid":"not-an-int","bad":1},"mismatches":0,"digest":"bad"}}
        ok, errors = validate_mutation_evidence(mutation, plan_digest="PLAN", reticulum_digest="RET")
        self.assertFalse(ok)
    ...
    def test_bootstrap_transport_uses_minimal_import_closure_when_possible(self):
        caps = {"RUNTIME_IMPORT":"UNAVAILABLE","REPOSITORY_READ":"AVAILABLE","PYTHON_EXECUTION":"AVAILABLE","SOURCE_TO_RUNTIME_BRIDGE":"AVAILABLE","SESSION_CONTEXT":"AVAILABLE"}
