...
class GradedExecutionV2Tests(unittest.TestCase):
    ...
    def test_contract_admission_and_standalone_prompt_binding(self):
        adm=json.loads((ROOT/'ADMISSION.json').read_text()); self.assertEqual(adm['contract_version'],'2.2.0'); self.assertEqual(adm['contract_semantic_revision'],'LOCAL_CHAT_STANDALONE_BOOTSTRAP_V1'); self.assertFalse(adm['method_access']['mandatory_profile_choice']); self.assertFalse(adm['host_runtime_transport']['lean_is_runtime_transport_class']); self.assertFalse(adm['local_cognitive_system']['load_before_acceptance']); self.assertFalse(adm['local_cognitive_system']['normative_host_nodes_replaced']); self.assertTrue(adm['local_cognitive_system']['standalone_boot_prompt']); self.assertFalse(adm['local_cognitive_system']['cognitive_companion_required']); self.assertEqual(adm['local_cognitive_system']['cognitive_policy'],adm['local_session_environment']['boot_prompt'])
    def test_five_normative_host_nodes_and_historical_pointer_only(self):
        adm=json.loads((ROOT/'ADMISSION.json').read_text()); self.assertEqual(len(adm['local_session_environment']['contract_nodes']),5); root=(ROOT/'docs/host/LOCAL_SESSION_ENVIRONMENT.md').read_text(); self.assertIn('historical compatibility pointer',root); self.assertIn('five host normative concerns',root)
