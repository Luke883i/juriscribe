...
class DashboardTests(unittest.TestCase):
    def test_dashboard_is_for_jurists_and_editorial_board(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            p=render_session_dashboard(state,Path(tmp)/'d.html'); text=p.read_text(encoding='utf-8'); self.assertIn('NON PRONTO',text); self.assertIn('Mappa epistemica',text); self.assertIn('Continuità monografica',text); self.assertIn('Simulazioni, saturazione e compressione finale',text); self.assertNotIn('chain-of-thought</strong>',text)
