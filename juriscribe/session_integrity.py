...
SESSION_INTEGRITY_SCHEMA = "juriscribe-session-integrity/v2"
...
def integrity_bindings(state):
    ...
    bindings = {
        ...
        "mode_selection_sha256": _digest(state.get("mode_selection", {})),
        "mode_contract_sha256": str((state.get("mode_contract") or {}).get("digest", "")),
        "editorial_standard_sha256": str((state.get("editorial_standard") or {}).get("digest", "")),
    ...
    # v1 continuity is additive. Historical sessions without the archive keep the
    # exact v2 integrity shape they already had; once continuity exists it is sealed.
    strategy = state.get("strategy") or {}
    continuity = strategy.get("continuity") if isinstance(strategy, dict) else None
    if isinstance(continuity, dict) and continuity:
        bindings["continuity_sha256"] = _digest(continuity)
    return bindings
...
def integrity_record(state):
    return {
        ...
        "legacy_projection": {
            ...
            "status": "MIGRATION_INPUT_ONLY",
            "required": False,
            "note": "node.h is read only to migrate pre-v0.8 workspaces; v0.9 does not generate or require it",
        },
    }


def render_session_integrity(state):
