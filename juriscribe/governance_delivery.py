...
from .browser_delivery import browser_docx_delivery_gate
...
def evaluate_completion(state):
    """Final delivery boundary: evidence, artifacts and DOCX delivery must converge."""
    ...
    docx_ok, docx_errors = browser_docx_delivery_gate(state)
    ...
    gate_results["browser_docx_delivery"] = (docx_ok, docx_errors)
    ...
    state.completion["browser_docx_delivery_gate"] = {"eligible": docx_ok, "errors": docx_errors}
        ...
        state.interaction = {
            **(state.interaction or {}),
            "card": interaction_card(
                ...
                summary="Configurazione, originalità, contenuto materializzato, controlli scientifici, copertura degli artefatti e download DOCX devono convergere allo stesso fixed point prima della consegna.",
                blocking=True,
            ),
            "status": "READY",
        }
