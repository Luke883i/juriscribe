...
def bootstrap_card(state: str, *, contract_version: str = "", detail: str = "") -> dict[str, Any]:
    ...
    cards = {
        ...
        "MODE_SELECTION_REQUIRED": (
            "Scegli la modalità Juriscribe",
            ["CONTINUATION", "GREENFIELD", "REVIEW", "ALTRO"],
        ),
...
def issue_probe_receipt(
    ...
    *,
    ...
    probe_nonce = probe_nonce or secrets.token_hex(16)
    if len(probe_nonce) != 32 or any(ch not in "0123456789abcdef" for ch in probe_nonce):
        raise ValueError("probe nonce must be 128-bit lowercase hex")
    payload = {
        "schema": PROBE_SCHEMA,
        "status": "PROBED",
        "admission_receipt_id": admission_receipt.get("receipt_id", ""),
        "admission_receipt_nonce": admission_receipt.get("receipt_nonce", ""),
...
def validate_probe_receipt(
    ...
    if probe_receipt.get("admission_receipt_nonce") != (admission_receipt or {}).get("receipt_nonce"):
        errors.append("probe receipt admission nonce mismatch")
    ...
    elif probe_receipt.get("capabilities_digest") != canonical_digest(
        {str(k): str(v) for k, v in sorted(caps.items())}
    ):
    ...
    if len(nonce) != 32 or any(ch not in "0123456789abcdef" for ch in nonce):
        errors.append("probe receipt nonce invalid")
    expected_id_payload = dict(probe_receipt)
    expected_id_payload.pop("receipt_id", None)
    expected_id = "PRB-" + canonical_digest(expected_id_payload)[:16]
    if probe_receipt.get("receipt_id") != expected_id:
        errors.append("probe receipt id/digest mismatch")
...
def claim_probe_receipt(root: str | Path, probe_receipt: dict[str, Any], session_id: str) -> Path:
    """Consume a sealed probe receipt once per workspace root.

    The exclusive-create marker prevents replaying the same host-capability snapshot
    to initialize a second session. It is deliberately fail-closed after a crash.
    """
    ...
    claims = Path(root) / ".bootstrap-claims"
    claims.mkdir(parents=True, exist_ok=True)
    marker = claims / f"{receipt_id}.json"
    payload = json.dumps(
        {
            "probe_receipt_id": receipt_id,
            "probe_nonce": probe_receipt.get("probe_nonce", ""),
    ...
    try:
        ...
        try:
            ...
            owner = prior.get("session_id", "unknown")
        except Exception:
            owner = "unknown"
...
def activate_work(admission: dict[str, Any], *, contract_version: str) -> None:
    admission["bootstrap"] = bootstrap_card(
        "ACTIVE_WORK",
        contract_version=contract_version,
        detail="Mode selected; substantive work is authorized under the mode contract.",
    )
...
def bootstrap_gate(admission: dict[str, Any] | None) -> tuple[bool, list[str]]:
    ...
    legacy_contract = str((admission.get("receipt") or {}).get("contract_version", "")) in {"1.5.0", "1.6.0"}
    if state not in {"MODE_SELECTION_REQUIRED", "ACTIVE_WORK"} and not (state == "ACTIVE" and legacy_contract):
        errors.append("bootstrap is not ready for mode selection or active work")
