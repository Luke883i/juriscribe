...
Historical commands delegate to pipeline_v9. C&C adds only mode-specific commands;
the existing fast bootstrap is patched only to derive choices from canonical modes.
"""
...
from .runtime_v11 import calibrate_refactoring, record_consolidation_saturation, register_refactoring_plan, seal_refined_candidate, consolidation_gate

_ORIGINAL_FAST_BOOTSTRAP = _v9.bootstrap_after_acceptance
def _dynamic_fast_bootstrap(*args, **kwargs):
    result=_ORIGINAL_FAST_BOOTSTRAP(*args,**kwargs)
    result["choices"]=[*mode_choices(),"ALTRO"]
    return result
_v9.bootstrap_after_acceptance=_dynamic_fast_bootstrap
...
def _workspace(session_dir): p=Path(session_dir); return Workspace(p.parent,p.name)
...
def _cc_main(argv):
    ...
    elif args.command=="consolidation-seal-refined": out=seal_refined_candidate(state,source_id=args.source_id,text=Path(args.text_file).read_text(encoding="utf-8"),semantic_recall=args.semantic_recall,relation_recall=args.relation_recall)
    else:
