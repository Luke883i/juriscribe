...
def _strategy(state: Any) -> dict[str, Any]:
    s = _payload(state)
    return s.setdefault("strategy", {})


def get_pipeline_lock(state: Any) -> dict[str, Any]:
    return dict((_strategy(state) or {}).get("natural_language_pipeline") or {})


def _set_contract(state: Any, contract: dict[str, Any]) -> None:
    _strategy(state)["natural_language_pipeline"] = contract
...
def initialize_pipeline_lock(state: Any) -> dict[str, Any]:
    ...
    existing = get_pipeline_lock(state)
    lock = {
        "schema": SCHEMA,
        "profile": PROFILE_ID,
        "status": "LOCKED",
        ...
        "interpretations": list(existing.get("interpretations") or []),
        "policy": {
...
def pipeline_lock_gate(state: Any) -> tuple[bool, list[str]]:
    ...
    lock = get_pipeline_lock(state)
    if not lock:
        # Migration compatibility: sessions created before v0.10.0 have no language
        # contract. New sessions reach mode selection through runtime_autopilot and
        # therefore always carry this marker.
        return True, []
    errors: list[str] = []
    if lock.get("schema") != SCHEMA or lock.get("profile") != PROFILE_ID:
        errors.append("natural-language pipeline contract schema/profile mismatch")
    ...
    if lock.get("locked_mode") != mode: errors.append("natural-language pipeline lock drift: mode")
    if lock.get("locked_primary_artifact_role") != expected_primary: errors.append("natural-language pipeline lock drift: primary artifact")
    if lock.get("locked_required_artifact_roles") != expected_roles: errors.append("natural-language pipeline lock drift: standard artifact set")
    if lock.get("mode_selection_digest") != str((s.get("mode_selection") or {}).get("digest") or ""): errors.append("natural-language pipeline lock drift: mode selection")
    if lock.get("lock_digest") != canonical_digest({k: v for k, v in lock.items() if k != "lock_digest"}): errors.append("natural-language pipeline lock digest mismatch")
    for item in lock.get("interpretations") or []:
        ...
        if item.get("pipeline_effect") not in PIPELINE_EFFECTS: errors.append(f"invalid pipeline effect in natural-language interpretation: {item.get('id')}")
        if item.get("classification") not in CLASSIFICATIONS: errors.append(f"invalid natural-language classification: {item.get('id')}")
...
def record_natural_language_interpretation(state: Any, utterance: str, interpretation: dict[str, Any]) -> dict[str, Any]:
    ...
    status = "APPLIED"; effect = proposed_effect; errors: list[str] = []
    if classification in {"MODE_CHANGE_REQUEST", "NEW_WORK_REQUEST"}:
        effect = "NEW_SESSION_REQUIRED"; status = "BLOCKED"; errors.append("mode/new-work change cannot mutate the active pipeline implicitly")
    elif classification == "AMBIGUOUS":
        effect = "BLOCKED_PENDING_CLARIFICATION"; status = "AMBIGUOUS"; errors.append("ambiguous material language requires clarification before state mutation")
    elif forbidden_requested:
        effect = "BLOCKED_PENDING_CLARIFICATION"; status = "BLOCKED"; errors.append("natural language cannot replace mode, primary artifact, standard artifacts, output format or pipeline steps")
    elif classification in {"STATUS_QUERY", "OUT_OF_SCOPE"}:
        effect = "NONE"
    interpretations = list(lock.get("interpretations") or [])
    record = {
        "id": f"NL-{len(interpretations)+1:04d}",
    ...
    interpretations.append(record); lock["interpretations"] = interpretations
    lock["lock_digest"] = canonical_digest({k: v for k, v in lock.items() if k != "lock_digest"}); _set_contract(state, lock)
    return record


def resolve_natural_language_interpretation(state: Any, record_id: str, resolution: str) -> dict[str, Any]:
    lock = get_pipeline_lock(state); records = list(lock.get("interpretations") or [])
    target = next((item for item in records if str(item.get("id")) == str(record_id)), None)
    if not target: raise ValueError("natural-language interpretation record not found")
    target["resolution_status"] = "RESOLVED"; target["resolution"] = " ".join(str(resolution or "").split())[:1000]
    target["digest"] = canonical_digest({k: v for k, v in target.items() if k != "digest"})
    lock["interpretations"] = records; lock["lock_digest"] = canonical_digest({k: v for k, v in lock.items() if k != "lock_digest"}); _set_contract(state, lock)
    return target


def build_final_artifact_inference_trace(state: Any, role: str, candidate_digest: str) -> dict[str, Any]:
    s = _payload(state); lock = get_pipeline_lock(state)
    material_interpretations = [item for item in lock.get("interpretations") or [] if item.get("classification") in MATERIAL_CLASSIFICATIONS and item.get("status") == "APPLIED"]
    ...
    trace = {
        "schema": TRACE_SCHEMA, "profile": PROFILE_ID, "status": "PASS", "artifact_role": str(role),
        "request_id": str((s.get("request") or {}).get("request_id") or ""), "mode": str(s.get("mode") or ""),
        ...
        "material_epistemic_unit_ids": unit_ids, "material_claim_ids": claim_ids,
        ...
        "candidate_digest": str(candidate_digest or ""), "required_artifact_roles": list(lock.get("locked_required_artifact_roles") or []),
    }
    errors: list[str] = []
    ok_lock, lock_errors = pipeline_lock_gate(s)
    if not ok_lock: errors.extend(lock_errors)
    ...
    if not str(candidate_digest or ""): errors.append("final artifact inference trace requires candidate digest")
    trace["status"] = "PASS" if not errors else "FAIL"; trace["errors"] = list(dict.fromkeys(errors))
    trace["digest"] = canonical_digest({k: v for k, v in trace.items() if k != "digest"}); return trace


def final_chapter_inference_trace_gate(state: Any) -> tuple[bool, list[str]]:
    s = _payload(state)
    if str(s.get("mode") or "") != "CONTINUATION" or not get_pipeline_lock(state): return True, []
    artifact = next((item for item in s.get("artifacts") or [] if item.get("role") == "final_chapter"), None)
    if not artifact: return False, ["final_chapter artifact missing inference trace"]
    trace = artifact.get("inference_trace") or {}; errors: list[str] = []
    if trace.get("schema") != TRACE_SCHEMA or trace.get("profile") != PROFILE_ID: errors.append("final_chapter inference trace schema/profile mismatch")
    if trace.get("status") != "PASS": errors.extend(trace.get("errors") or ["final_chapter inference trace is not PASS"])
    current_candidate = str((s.get("drafts") or [{}])[-1].get("digest") or "") if s.get("drafts") else ""
    if trace.get("candidate_digest") != current_candidate: errors.append("final_chapter inference trace is bound to stale candidate")
    if trace.get("generation_contract_digest") != str((s.get("generation_contract") or {}).get("contract_digest") or ""): errors.append("final_chapter inference trace is bound to stale generation contract")
    if trace.get("digest") != canonical_digest({k: v for k, v in trace.items() if k != "digest"}): errors.append("final_chapter inference trace digest mismatch")
    return not errors, list(dict.fromkeys(errors))
