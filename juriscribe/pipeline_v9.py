...
from .orchestrator import (
    apply_setup, audit_legal_text, build_research_plan, evaluate_completion, freeze_dods,
    ...
    record_review_saturation, record_simulation, register_bibliography,
    register_continuation_plan, register_semantic_mining, seal_draft, select_mode,
...
def probe_capabilities(*, admission_receipt=None, contract_text=None):
    ...
    checks = {
        "SESSION_CONTEXT": "AVAILABLE", "LOCAL_SCRATCH_IO": "UNVERIFIED",
        "STRUCTURED_STORAGE": "AVAILABLE", "ATTACHMENT_READ": "UNVERIFIED",
        "DOCX_READ": "UNVERIFIED", "DOCX_WRITE": "UNVERIFIED",
        "DOCX_READBACK": "UNVERIFIED", "PDF_READ": "UNVERIFIED",
        "WEB_RESEARCH": "UNVERIFIED", "REPOSITORY_READ": "UNVERIFIED",
        "REPOSITORY_WRITE": "UNVERIFIED", "CLOCK": "AVAILABLE", "HASHING": "AVAILABLE",
    }
    try:
...
def perform_probe(*, admission_receipt, contract_text=None, host_capabilities=None, host=None, probed_at=None):
    ...
    return issue_probe_receipt(
        admission_receipt, contract_text, caps,
        host=host or platform.platform(), probed_at=probed_at,
    )
...
def initialize(request, root=".juriscribe", session_id=None, host_capabilities=None, *, admission_receipt=None, probe_receipt=None, contract_text=None):
    ...
    state.interaction = {
        "card": interaction_card(
            "MODE_SELECTION_REQUIRED",
            summary="Scegli CONTINUATION, GREENFIELD o REVIEW prima di caricare i materiali sostanziali.",
        ),
        "history": [], "status": "READY",
    }
    state.artifacts.append({
        "id": "session-integrity", "role": "session_integrity",
...
def bootstrap_after_acceptance(request, *, phrase, actor_type, evidence_type, user_message, root=".juriscribe", session_id=None, host_capabilities=None, contract_text=None):
    """Fast path after explicit human acceptance.

    Admission, probe and initialize remain distinct runtime steps/receipts but are
    orchestrated in one host turn; mode selection is still a separate human choice.
    """
    contract_text = contract_text or load_contract_text()
    receipt = issue_receipt(
        contract_text, phrase=phrase, actor_type=actor_type,
        evidence_type=evidence_type, user_message=user_message,
    )
    probe = perform_probe(
        admission_receipt=receipt, contract_text=contract_text,
        host_capabilities=host_capabilities,
    )
    base = initialize(
        request, root=root, session_id=session_id,
        admission_receipt=receipt, probe_receipt=probe,
        contract_text=contract_text,
    )
    return {"session_dir": str(base), "state": "MODE_SELECTION_REQUIRED", "choices": ["CONTINUATION", "GREENFIELD", "REVIEW", "ALTRO"]}
...
def main(argv=None):
    ...
    integrity = sub.add_parser("integrity"); integrity.add_argument("session_dir")
    ...
    elif args.command == "semantic-mining": output = register_semantic_mining(state, _payload(args.json_file).get("units", []), _payload(args.json_file).get("relations", [])); rc = 0 if output.get("status") == "PASS" else 2
    ...
    elif args.command == "continuation-plan": output = register_continuation_plan(state, _payload(args.json_file))
    elif args.command == "continuation-coverage": output = record_continuation_coverage(state, _payload(args.json_file)); rc = 0 if output.get("status") == "PASS" else 2
    elif args.command == "research-plan": build_research_plan(state); output = state.source_intelligence["research_plan"]
    elif args.command == "validate-claims": output = validate_claim_ledger(state); rc = 1 if output else 0
    elif args.command == "seal-draft": output = seal_draft(state, Path(args.text_file).read_text(encoding="utf-8"), stage=args.stage)
    ...
    elif args.command == "review-saturation": output = record_review_saturation(state, _payload(args.json_file)); rc = 0 if output.get("status") == "PASS" else 2
    ...
    elif args.command == "record-provenance": output = record_provenance(state, _payload(args.json_file))
    elif args.command == "final-review": output = record_final_review(state, _payload(args.json_file))
