...
def completion_gate(
    ...
    *,
    ...
    if not all_dods_done(dods):
        reasons.append("not all blocking DoD are DONE")
    if int(metrics.get("dod_no_novelty_streak", 0)) < 10000:
        reasons.append("M+10000 no-novelty evidence vs DoD not reached")
    if open_contra:
        reasons.append("blocking contradictions remain open")
    if quality and quality.get("status") != "PASS":
        reasons.append("chapter quality review/failure remains unresolved")
    if source_coverage is not None and source_coverage not in {"PASS", "NOT_REQUIRED"}:
        reasons.append("claim/source coverage is not closed")
    bg = benchmark_gate(benchmark, required=benchmark_required)
    if not bg["eligible"]:
        reasons.append("blind monograph benchmark integrity/coverage failed")
    if artifacts and [a for a in artifacts if a.get("required", True) and a.get("readback") != "PASS"]:
        reasons.append("required artifact readback failed")
    ...
    if generation_required:
        ...
        if not reticulum or reticulum.get("status") != "PASS":
            reasons.append("validated epistemic reticulum is required")
        ...
        if not final_artifacts:
            reasons.append("final chapter artifact is missing")
        elif any(a.get("readback") != "PASS" for a in final_artifacts):
            reasons.append("final chapter artifact readback failed")
        ...
        if not drafts:
            ...
            if current.get("stage") != "COMPRESSED_FINAL":
                reasons.append("current candidate is not the sealed COMPRESSED_FINAL draft")
            if not any(d.get("stage") == "INITIAL" for d in drafts):
        ...
        ok_comp, comp_errors = compression_valid(
            compression,
            expected_before_digest=precompression_digest or None,
            expected_after_digest=current_digest or None,
            generation_contract_digest=contract_digest,
            strict=True,
        )
        if not ok_comp:
            reasons.extend(comp_errors)

        ok_sim, sim_errors = validate_simulation_receipt(
            simulation,
            candidate_digest=current_digest or None,
            generation_contract_digest=contract_digest,
        ...
        if not ok_sim:
            reasons.extend(sim_errors)

        if not quality or quality.get("status") != "PASS":
            if "chapter quality review/failure remains unresolved" not in reasons:
                reasons.append("chapter quality PASS evidence is required")
