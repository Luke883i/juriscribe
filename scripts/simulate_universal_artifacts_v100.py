...
ASSISTANTS = (
    "OPENAI_ASSISTANT", "GENERIC_AI_ASSISTANT", "ENTERPRISE_AGENT", "LOCAL_MODEL_HOST", "UNKNOWN_ASSISTANT_HOST",
)
LANGUAGE_FAMILIES = (
    "BENIGN_CONTENT_CONSTRAINT",
    "SOFT_MODE_CHANGE",
    "SKIP_PIPELINE_REQUEST",
    "AMBIGUOUS_SHORTCUT",
    "STATUS_QUERY",
    "OUTPUT_FORMAT_DERAIL",
    "DISABLE_STANDARD_ARTIFACTS",
    "NEW_WORK_REQUEST",
    "MATERIAL_DECISION",
    "OUT_OF_SCOPE",
)
...
def _state(root: Path, mode: str, case_id: int, browser: str, assistant: str) -> SimpleNamespace:
    ...
    if mode == REVIEW:
        ...
        review = {
            "cycles": [{
                "cycle": 1, "status": "PASS",
                "findings": [{"finding_id": f"F-{case_id:03d}", "gravita": "MAJOR", "problema_rilevato": "Finding sintetico reale", "intervento_proposto": "Correzione documentata"}],
            }],
            "regenerations": [], "saturation": {"status": "PASS"}, "status": "DIAGNOSTIC_SATURATED",
        }
    state = SimpleNamespace(
        ...
        mode_selection={"digest": f"MODE-{case_id:03d}"}, mode_contract={}, editorial_standard={}, corpus=[],
        sources=[{"id": "S1", "title": "Fonte normativa scenario", "source_type": "primary_law", "direct_read": True, "verified_at": "2026-08-17"}],
        bibliography={}, epistemic_units=[{"id": "U1", "kind": "RULE", "text": f"Regola materiale scenario {case_id}", "source_id": "S1", "material": True, "status": "VERIFIED"}],
        relations=[], reticulum={"status": "PASS"}, generation_contract=generation_contract, continuation=continuation,
        drafts=drafts, review=review, final_review={"status": "PASS"}, provenance={}, contradictions=[], mining={}, style_profile={},
        source_intelligence={}, claim_ledger=[{"id": "C1", "text": f"Claim materiale scenario {case_id}", "material": True}], artifact_evidence=[],
...
def _apply_language_case(state: SimpleNamespace, family: str) -> tuple[str, bool]:
    if family == "BENIGN_CONTENT_CONSTRAINT":
        record = record_natural_language_interpretation(state, "Approfondisci questo punto, senza cambiare il capitolo", {
            "classification": "CONTENT_CONSTRAINT", "pipeline_effect": "CONTENT_CONSTRAINT",
            "interpretation": "Vincolo interno al prodotto già selezionato", "affected_unit_ids": ["U1"],
        })
    ...
    if family == "SOFT_MODE_CHANGE":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Nuova sessione necessaria; pipeline corrente invariata")
        return "MODE_CHANGE_BLOCKED", record["status"] == "BLOCKED"
    if family == "SKIP_PIPELINE_REQUEST":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Gate obbligatori preservati")
        return "PIPELINE_SKIP_BLOCKED", record["status"] == "BLOCKED"
    if family == "AMBIGUOUS_SHORTCUT":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Nessuna mutazione materiale applicata")
        return "AMBIGUITY_BLOCKED", record["status"] == "AMBIGUOUS"
    if family == "STATUS_QUERY":
        record = record_natural_language_interpretation(state, "A che punto siamo?", {
            "classification": "STATUS_QUERY", "pipeline_effect": "NONE", "interpretation": "Richiesta di stato",
        })
        return "STATUS_QUERY_ISOLATED", record["pipeline_effect"] == "NONE"
    if family == "OUTPUT_FORMAT_DERAIL":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Formato standard DOCX preservato")
        return "OUTPUT_FORMAT_DERAIL_BLOCKED", record["status"] == "BLOCKED"
    if family == "DISABLE_STANDARD_ARTIFACTS":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Set standard preservato")
        return "STANDARD_ARTIFACT_SUPPRESSION_BLOCKED", record["status"] == "BLOCKED"
    if family == "NEW_WORK_REQUEST":
        ...
        resolve_natural_language_interpretation(state, record["id"], "Nuova sessione separata necessaria")
        return "NEW_WORK_ISOLATED", record["status"] == "BLOCKED"
    ...
    record = record_natural_language_interpretation(state, "A proposito, che tempo fa?", {
        "classification": "OUT_OF_SCOPE", "pipeline_effect": "NONE", "interpretation": "Richiesta estranea al lavoro",
    })
...
def _scenario(root: Path, ordinal: int, *, extension: bool) -> dict:
    ...
    if not language_ok:
        raise AssertionError(f"natural-language contract failed in scenario {ordinal}: {family}")
    lock_ok, lock_errors = pipeline_lock_gate(state)
    if not lock_ok:
        raise AssertionError(f"pipeline lock not converged in scenario {ordinal}: {lock_errors}")
    receipt = materialize_standard_artifacts(state)
    if receipt.get("status") != "PASS":
        raise AssertionError(f"artifact autopilot failed in scenario {ordinal}: {receipt}")
    auto_ok, auto_errors = standard_artifact_autopilot_gate(state)
    if not auto_ok:
        raise AssertionError(f"artifact autopilot gate failed in scenario {ordinal}: {auto_errors}")
    expected = sorted(required_artifact_roles(mode, state.setup) - {"session_dashboard"})
    if receipt.get("materialized_roles") != expected:
        raise AssertionError(f"standard artifact set drift in scenario {ordinal}: {receipt}")
    if not all(str(item.get("path") or "").lower().endswith(".docx") and item.get("auto_materialized_by_runtime") for item in state.artifacts):
        raise AssertionError(f"non-DOCX or non-runtime-owned artifact in scenario {ordinal}")
    chat = build_chat_delivery_manifest(state)
    if chat.get("status") != "PASS" or chat.get("placement") != "SESSION_CHAT_TAIL":
        raise AssertionError(f"chat-tail manifest failed in scenario {ordinal}: {chat}")
    ...
    if isolation.get("status") != "PASS":
        raise AssertionError(f"dashboard leaked document downloads in scenario {ordinal}: {isolation}")
    if 'id="chat-tail-delivery-summary"' not in page:
        raise AssertionError(f"dashboard lacks synthetic artifact summary in scenario {ordinal}")
    if mode == CONTINUATION:
        chapter = next(item for item in state.artifacts if item.get("role") == "final_chapter")
        if (chapter.get("inference_trace") or {}).get("status") != "PASS":
            raise AssertionError(f"final_chapter trace failed in scenario {ordinal}")
    return {
        ...
        "learned_category": category, "artifact_count": len(expected), "status": "PASS",
    }
...
def run(cases: int = M, no_novelty: int = NO_NOVELTY_EXTENSION, out_root: str | None = None) -> dict:
    ...
    try:
        ...
        if any(not row["browser_context"].startswith("SAFARI") for row in primary):
            raise AssertionError("all 100 primary scenarios must be Safari-targeted")
        ...
        if novel:
            raise AssertionError("M+100 produced novel pipeline/artifact categories: " + ", ".join(novel))
        rows = primary + extension
        return {
            "schema": SCHEMA, "status": "PASS", "M": cases, "one_to_M_cases": len(primary),
            "M_plus_100_cases": len(extension), "total_cases": len(rows), "safari_primary_cases": len(primary),
            "no_novelty_after_M": not novel, "novel_categories_after_M": novel,
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--cases", type=int, default=M)
