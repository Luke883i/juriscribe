...
def main():
    contract=(ROOT/'ISENECA_ACCESS_CONTRACT.md').read_text(encoding='utf-8'); admission=json.loads((ROOT/'ADMISSION.json').read_text(encoding='utf-8')); manifest=json.loads((ROOT/'MANIFEST.json').read_text(encoding='utf-8')); agents=(ROOT/'AGENTS.md').read_text(encoding='utf-8'); pipeline=(ROOT/'juriscribe/pipeline.py').read_text(encoding='utf-8'); workflow=(ROOT/'.github/workflows/runtime-regression.yml').read_text(encoding='utf-8')
    ...
    if 'issue_receipt(contract' not in pipeline or "if args.command=='accept'" not in pipeline: fail('explicit accept command missing')
    ...
    for required in ['python -m unittest discover -s tests -v','python scripts/check_contract.py','python scripts/simulate_v4.py','python scripts/reflect_v4.py']:
        if required not in workflow: fail(f'CI missing required gate: {required}')
    print(json.dumps({'status':'PASS','runtime_version':__version__,'contract_version':CONTRACT_VERSION,'contract_sha256':admission['contract_sha256'],'pre_admission_allowlist':admission['pre_admission_allowlist']},indent=2)); return 0
