...
def main():
    ...
    if mode_choices()!=expected or admission.get('canonical_modes')!=expected or (manifest.get('modes') or {}).get('canonical')!=expected: fail('canonical mode list mismatch')
    if schema.get('properties',{}).get('mode',{}).get('enum')!=expected: fail('mode schema canonical enum mismatch')
    cc=manifest.get('compression_consolidation') or {}
    required={'structural_semantic_proof_profile':SEMANTIC_PROOF_PROFILE,'structural_semantic_proof_claim_scope':CLAIM_SCOPE,'mutation_instance_claim_scope':INSTANCE_CLAIM_SCOPE,'saturation_instance_claim_scope':INSTANCE_CLAIM_SCOPE,'caller_supplied_recall_forbidden':True,'mutation_equivalence_class_evidence_required':True,'saturation_equivalence_class_evidence_required':True}
    for key,value in required.items():
        if cc.get(key)!=value: fail('compression/consolidation manifest invariant mismatch: '+key)
    if cc.get('semantic_truth_claim') is not False or cc.get('legal_entailment_claim') is not False: fail('structural proof overstated')
    transport=admission.get('host_runtime_transport') or {}
    if transport.get('bootstrap_minimal_materialization_allowed') is not True: fail('minimal bootstrap transport not enabled')
    if tuple(transport.get('bootstrap_source_paths') or ())!=BOOTSTRAP_SOURCE_PATHS: fail('bootstrap source closure mismatch')
    if transport.get('deferred_full_runtime_after_bootstrap') is not True: fail('full runtime is not deferred')
    if 'BOOTSTRAP_SOURCE_PATHS' not in host or 'SINGLE_HOST_TURN_AFTER_ACCEPTANCE' not in host: fail('host bootstrap hardening missing')
    if '--projection-json' not in pipeline or '--semantic-recall' in pipeline or '--relation-recall' in pipeline: fail('public seal command accepts caller recall')
    for token in ['verify_structural_semantic_proof','validate_mutation_evidence','validate_saturation_evidence']:
        if token not in runtime: fail('runtime v0.12 missing '+token)
    active=manifest.get('active_surface') or {}
    if active.get('policy')!='CURRENT_ONLY_UNLESS_COMPATIBILITY_AUDIT' or active.get('historical_docs')!='compatibility-and-audit-only': fail('historical surface not demoted')
    packaging=manifest.get('packaging') or {}
    if packaging.get('standard')!='PEP_517_PYPROJECT' or packaging.get('bundled_contract_resource')!='juriscribe/resources/ISENECA_ACCESS_CONTRACT.md': fail('portable package contract missing')
    print('RUNTIME SEMANTICS CONTRACT PASS'); return 0
